...
minimize Cost: sum{e in Links} (link_cost[e]*u[e]*M) + sum{v in Nodes} (Node_cost[v]*z[v]);	 
...
subj to Node_capacity {v1 in Nodes}:
sum {v2 in Nodes diff {v1}} (sum{e in Links} b[e,v2]*x[e,v1]) <= z[v1]; 